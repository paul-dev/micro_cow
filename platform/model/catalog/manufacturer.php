...
class ModelCatalogManufacturer extends Model {
	...
	public function getManufacturer($manufacturer_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'manufacturer_id=" . (int)$manufacturer_id . "') AS keyword FROM " . DB_PREFIX . "manufacturer m LEFT JOIN " . DB_PREFIX . "manufacturer_to_store m2s ON m.manufacturer_id = m2s.manufacturer_id WHERE m2s.store_id = '".(int)$this->config->get('config_store_id')."' AND m.manufacturer_id = '" . (int)$manufacturer_id . "'");
