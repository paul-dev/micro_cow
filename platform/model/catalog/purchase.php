...
class ModelCatalogPurchase extends Model {
	...
	public function getPurchase($purchase_id) {
		...
		$recommend = $this->db->query("SELECT status FROM " . DB_PREFIX . "purchase_to_recommend WHERE purchase_id = '".$purchase_id."'");
		$latest = $this->db->query("SELECT status FROM " . DB_PREFIX . "purchase_to_latest WHERE purchase_id = '".$purchase_id."'");
		$query->row['recommend'] = $recommend->row['status'];
		$query->row['latest'] = $latest->row['status'];
	...
	public function getPurchases($data = array()) {
		...
		if ($data['recommend'] == 1 && $data['latest'] == 0) {
			$sql = "SELECT p.*, pd.*, cd.company_name, (SELECT COUNT(*) as total FROM " . DB_PREFIX . "purchase_offer po WHERE po.purchase_id = p.purchase_id ) as total_offer, " .
					...
					"INNER JOIN " . DB_PREFIX . "purchase_to_recommend ptr on p.purchase_id = ptr.purchase_id " .
					...
					"WHERE ps.store_id = '".(int)$this->config->get('config_store_id')."' AND ptr.status = 1 AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		}elseif ($data['latest'] == 1 && $data['recommend'] == 0) {
			$sql = "SELECT p.*, pd.*, cd.company_name, (SELECT COUNT(*) as total FROM " . DB_PREFIX . "purchase_offer po WHERE po.purchase_id = p.purchase_id ) as total_offer, " .
					"(SELECT IF(COUNT(*) > 1, CONCAT(COUNT(*), '种'), (SELECT CONCAT(pp1.quantity,pd1.unit) as p_amount  FROM " . DB_PREFIX . "purchase_product pp1 LEFT JOIN " . DB_PREFIX . "purchase_product_description pd1 ON pp1.purchase_product_id = pd1.purchase_product_id WHERE pp1.purchase_id = p.purchase_id AND pd1.language_id = '".(int)$this->config->get('config_language_id')."')) as total FROM " . DB_PREFIX . "purchase_product pp WHERE pp.purchase_id = p.purchase_id) as total_product " .
					"FROM " . DB_PREFIX . "purchase p LEFT JOIN " . DB_PREFIX . "purchase_description pd ON (p.purchase_id = pd.purchase_id) " .
					"INNER JOIN " . DB_PREFIX . "purchase_to_latest ptl on p.purchase_id = ptl.purchase_id " .
					"LEFT JOIN " . DB_PREFIX . "purchase_to_store ps on p.purchase_id = ps.purchase_id " .
					"LEFT JOIN " . DB_PREFIX . "customer c on c.customer_id = p.customer_id " .
					"LEFT JOIN " . DB_PREFIX . "company cp on cp.customer_id = c.customer_id " .
					"LEFT JOIN " . DB_PREFIX . "company_description cd on cd.company_id = cp.company_id AND cd.language_id = '" . (int)$this->config->get('config_language_id') ."' " .
					"WHERE ps.store_id = '".(int)$this->config->get('config_store_id')."' AND ptl.status = 1 AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		}elseif ($data['recommend'] == 1 && $data['latest'] == 1) {
			$sql = "SELECT p.*, pd.*, cd.company_name, (SELECT COUNT(*) as total FROM " . DB_PREFIX . "purchase_offer po WHERE po.purchase_id = p.purchase_id ) as total_offer, " .
					"(SELECT IF(COUNT(*) > 1, CONCAT(COUNT(*), '种'), (SELECT CONCAT(pp1.quantity,pd1.unit) as p_amount  FROM " . DB_PREFIX . "purchase_product pp1 LEFT JOIN " . DB_PREFIX . "purchase_product_description pd1 ON pp1.purchase_product_id = pd1.purchase_product_id WHERE pp1.purchase_id = p.purchase_id AND pd1.language_id = '".(int)$this->config->get('config_language_id')."')) as total FROM " . DB_PREFIX . "purchase_product pp WHERE pp.purchase_id = p.purchase_id) as total_product " .
					"FROM " . DB_PREFIX . "purchase p LEFT JOIN " . DB_PREFIX . "purchase_description pd ON (p.purchase_id = pd.purchase_id) " .
					"INNER JOIN " . DB_PREFIX . "purchase_to_recommend ptr on p.purchase_id = ptr.purchase_id " .
					...
					"LEFT JOIN " . DB_PREFIX . "company cp on cp.customer_id = c.customer_id " .
					"LEFT JOIN " . DB_PREFIX . "company_description cd on cd.company_id = cp.company_id AND cd.language_id = '" . (int)$this->config->get('config_language_id') ."' " .
					"WHERE ps.store_id = '".(int)$this->config->get('config_store_id')."' AND ptr.status = 1 AND ptl.status = 1 AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		}else{
			$sql = "SELECT p.*, pd.*, cd.company_name, (SELECT COUNT(*) as total FROM " . DB_PREFIX . "purchase_offer po WHERE po.purchase_id = p.purchase_id ) as total_offer, " .
					"(SELECT IF(COUNT(*) > 1, CONCAT(COUNT(*), '种'), (SELECT CONCAT(pp1.quantity,pd1.unit) as p_amount  FROM " . DB_PREFIX . "purchase_product pp1 LEFT JOIN " . DB_PREFIX . "purchase_product_description pd1 ON pp1.purchase_product_id = pd1.purchase_product_id WHERE pp1.purchase_id = p.purchase_id AND pd1.language_id = '".(int)$this->config->get('config_language_id')."')) as total FROM " . DB_PREFIX . "purchase_product pp WHERE pp.purchase_id = p.purchase_id) as total_product " .
					"FROM " . DB_PREFIX . "purchase p LEFT JOIN " . DB_PREFIX . "purchase_description pd ON (p.purchase_id = pd.purchase_id) " .
	...
	public function getTotalPurchases($data = array()) {
		...
		if ($data['recommend'] == 1 && $data['latest'] == 0) {
			$sql .= " INNER JOIN " . DB_PREFIX . "purchase_to_recommend ptr ON p.purchase_id = ptr.purchase_id WHERE ptr.status = 1 AND ps.store_id = '".(int)$this->config->get('config_store_id')."' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		}elseif ($data['latest'] == 1 && $data['recommend'] == 0) {
			$sql .= " INNER JOIN " . DB_PREFIX . "purchase_to_latest ptl ON p.purchase_id = ptl.purchase_id WHERE ptl.status = 1 AND ps.store_id = '".(int)$this->config->get('config_store_id')."' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		}elseif ($data['recommend'] == 1 && $data['latest'] == 1) {
			$sql .= " INNER JOIN " . DB_PREFIX . "purchase_to_recommend ptr ON p.purchase_id = ptr.purchase_id "." INNER JOIN " . DB_PREFIX . "purchase_to_latest ptl ON p.purchase_id = ptl.purchase_id WHERE ptl.status = 1 AND ptr.status = 1 AND ps.store_id = '".(int)$this->config->get('config_store_id')."' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		}else{
