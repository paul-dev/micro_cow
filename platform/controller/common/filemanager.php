...
class ControllerCommonFileManager extends Controller {
    public function index() {
        ...
        foreach ($images as $image) {
            ...
            if (is_dir($image)) {
                ...
                $data['images'][] = array(
                    ...
                    'href'  => $this->url->link('common/filemanager', 'token='.$this->session->data['token'].'&directory=' . urlencode(utf8_substr($image, utf8_strlen($basePath . '/'))) . $url, 'SSL')
                );
        ...
        $this->response->setOutput($this->load->view('common/'.$template.'.tpl', $data));
    }
    ...
    protected function validateBasePath() {
        $directory = DIR_IMAGE . 'catalog/shop_image/' .md5($this->config->get('config_store_id'));
        if (!is_dir($directory)) {
