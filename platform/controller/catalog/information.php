...
class ControllerCatalogInformation extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $information_id) {
                $_check = $this->model_catalog_information->getInformation($information_id);
                if ($_check) $this->model_catalog_information->deleteInformation($information_id);
			}
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['is_notice'])) {
            $data['is_notice'] = $this->request->post['is_notice'];
        } elseif (!empty($information_info)) {
            $data['is_notice'] = $information_info['is_notice'];
        } else {
            $data['is_notice'] = 0;
        }

        if (isset($this->request->post['is_news'])) {
            $data['is_news'] = $this->request->post['is_news'];
        } elseif (!empty($information_info)) {
            $data['is_news'] = $information_info['is_news'];
        } else {
            $data['is_news'] = 0;
        }
