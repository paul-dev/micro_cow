...
class ControllerProductSearchProduct extends Controller {
    ...
    public function index() {
        ...
        $data['search_url'] = $this->url->link('product/search_product');

        if (isset($this->request->get['type']) && in_array($this->request->get['type'], $this->allow_type)) {
            $type = $this->request->get['type'];
            $data['search_url'] = $this->url->link('product/search_product', 'type='. $type);
        } else {
            $type = '';
        }
        ...
        if ($type) {
            ...
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('product/search_product', 'type='.$type)
            );
            $data['heading_title'] = '';
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('product/search_product', $url)
            );
		...
		if (1 == 1 || !empty($type) || isset($this->request->get['search']) || isset($this->request->get['tag'])) {
            ...
            $data['sorts'][] = array(
                ...
                'value' => 'p.viewed-DESC',
                'href'  => $this->url->link('product/search_product', '&sort=p.viewed&order=DESC' . $url)
            );
            $data['sorts'][] = array(
                'text'  => $this->language->get('text_sell_desc'),
                'value' => 'total_sell-DESC',
                'href'  => $this->url->link('product/search_product', '&sort=total_sell&order=DESC' . $url)
            );
            $data['sorts'][] = array(
                'text'  => $this->language->get('text_date_desc'),
                'value' => 'p.date_added-DESC',
                'href'  => $this->url->link('product/search_product', '&sort=p.date_added&order=DESC' . $url)
            );
			...
			foreach($limits as $value) {
				$data['limits'][] = array(
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('product/search_product', $url . '&limit=' . $value)
				);
			}
            ...
            $data['filter_prices'][] = array(
                'text'  => '0~1000',
                'value' => '0~1000',
                'href'  => $this->url->link('product/search_product', '&price=0~1000' . $url)
            );
            $data['filter_prices'][] = array(
                'text'  => '1001~5000',
                'value' => '1001~5000',
                'href'  => $this->url->link('product/search_product', '&price=1001~5000' . $url)
            );
            $data['filter_prices'][] = array(
                'text'  => '5001~10000',
                'value' => '5001~10000',
                'href'  => $this->url->link('product/search_product', '&price=5001~10000' . $url)
            );
            $data['filter_prices'][] = array(
                'text'  => '10001~100000',
                'value' => '10001~100000',
                'href'  => $this->url->link('product/search_product', '&price=10001~100000' . $url)
            );
            $data['filter_prices'][] = array(
                'text'  => '10万以上',
                'value' => '100001',
                'href'  => $this->url->link('product/search_product', '&price=100001' . $url)
            );
			...
			$pagination->url = $this->url->link('product/search_product', $url . '&page={page}');
            ...
            $data['results'] .= ' 到 <input type="text" name="input_page_jump" value="'.$page.'" maxlength="4" class="form-control" style="width: 30px; text-align: center; display: inline; line-height: 30px; padding: 0px; height: 30px;" /> 页 <button id="button-page-jump" data-url="'.$this->url->link('product/search_product', $url).'" class="btn btn-primary">Go</button>';
