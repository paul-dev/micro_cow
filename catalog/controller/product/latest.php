...
class ControllerProductLatest extends Controller {
    ...
    public function index() {
        ...
        $data['search_url'] = $this->url->link('product/latest');

        if (isset($this->request->get['type']) && in_array($this->request->get['type'], $this->allow_type)) {
            $type = $this->request->get['type'];
            $data['search_url'] = $this->url->link('product/latest', 'type='. $type);
        } else {
            $type = '';
        }
        ...
        if ($type) {
            ...
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('product/latest', 'type='.$type)
            );
            $data['heading_title'] = '';
        } else {
            $data['breadcrumbs'][] = array(
		...
		if (1 == 1 || !empty($type) || isset($this->request->get['search']) || isset($this->request->get['tag'])) {
            ...
            $data['sorts'][] = array(
                ...
                'href'  => $this->url->link('product/latest', '&sort=total_sell&order=DESC' . $url)
            );
            $data['sorts'][] = array(
            ...
            $data['filter_prices'][] = array(
                'text'  => '0~1000',
                'value' => '0~1000',
                'href'  => $this->url->link('product/latest', '&price=0~1000' . $url)
            );
            $data['filter_prices'][] = array(
                'text'  => '1001~5000',
                'value' => '1001~5000',
                'href'  => $this->url->link('product/latest', '&price=1001~5000' . $url)
            );
            $data['filter_prices'][] = array(
                'text'  => '5001~10000',
                'value' => '5001~10000',
                'href'  => $this->url->link('product/latest', '&price=5001~10000' . $url)
            );
            $data['filter_prices'][] = array(
                'text'  => '10001~100000',
                'value' => '10001~100000',
                'href'  => $this->url->link('product/latest', '&price=10001~100000' . $url)
            );
            $data['filter_prices'][] = array(
                'text'  => '10万以上',
                'value' => '100001',
                'href'  => $this->url->link('product/latest', '&price=100001' . $url)
            );
            ...
            $data['results'] .= ' 到 <input type="text" name="input_page_jump" value="'.$page.'" maxlength="4" class="form-control" style="width: 30px; text-align: center; display: inline; line-height: 30px; padding: 0px; height: 30px;" /> 页 <button id="button-page-jump" data-url="'.$this->url->link('product/latest', $url).'" class="btn btn-primary">Go</button>';
