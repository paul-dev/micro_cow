...
class ControllerSellerCategory extends Controller {
	...
	public function add() {
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('seller/category/add', '', 'SSL');
		...
		$this->document->setTitle($this->language->get('text_add').' - '.$this->language->get('seller_home'));
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $url = trim($url, '&');
			$this->response->redirect($this->url->link('seller/category', $url, 'SSL'));
		}
	...
	public function edit() {
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('seller/category/edit', 'category_id='.$this->request->get['category_id'], 'SSL');
		...
		$this->document->setTitle($this->language->get('text_edit').' - '.$this->language->get('seller_home'));

		$this->load->model('seller/category');

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $category_info = $this->model_seller_category->getCategory($this->request->get['category_id']);
	...
	public function repair() {
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('seller/category/repair', '', 'SSL');
		...
		//if ($this->validateRepair()) {
			$this->model_seller_category->repairCategories();
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('seller/category/add', $url, 'SSL');
		$data['delete'] = $this->url->link('seller/category/delete', $url, 'SSL');
		$data['repair'] = $this->url->link('seller/category/repair', $url, 'SSL');
		...
		$category_total = $this->model_seller_category->getTotalCategories();
		...
		foreach ($results as $result) {
			$data['categories'][] = array(
				...
				'edit'        => $this->url->link('seller/category/edit', 'category_id=' . $result['category_id'] . $url, 'SSL'),
	...
	protected function getForm() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('seller/category', $url, 'SSL')
		);
		
		if (!isset($this->request->get['category_id'])) {
			$data['action'] = $this->url->link('seller/category/add', $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('seller/category/edit', 'category_id=' . $this->request->get['category_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('seller/category', $url, 'SSL');
		...
		if (isset($this->request->post['category_filter'])) {
			...
			$filters = $this->model_seller_category->getCategoryFilters($this->request->get['category_id']);
		} else {
			$filters = array();
		}
	...
	protected function validateForm() {
		/*if (!$this->user->hasPermission('modify', 'seller/category')) {
