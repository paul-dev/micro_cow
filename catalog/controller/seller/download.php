...
class ControllerSellerDownload extends Controller {
	...
	public function add() {
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('seller/download/add', '', 'SSL');
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('seller/download/add', $url, 'SSL');
		...
		$download_total = $this->model_seller_download->getTotalDownloads($filter_data);

		$results = $this->model_seller_download->getDownloads($filter_data);
		...
		foreach ($results as $result) {
			$data['downloads'][] = array(
				...
				'edit'        => $this->url->link('seller/download/edit', 'download_id=' . $result['download_id'] . $url, 'SSL')
			);
		}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['download_id'])) {
			$data['action'] = $this->url->link('seller/download/add', $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('seller/download/edit', 'download_id=' . $this->request->get['download_id'] . $url, 'SSL');
		}
	...
	protected function validateForm() {
		/*if (!$this->user->hasPermission('modify', 'seller/download')) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) && $this->customer->isSeller()) {
			$this->load->model('seller/download');
