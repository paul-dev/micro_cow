...
class ControllerSellerReturn extends Controller {
	...
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('seller/return', '', 'SSL');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('seller/return', $url, 'SSL')
		);
		...
		$return_total = $this->model_seller_return->getTotalReturns();

		$results = $this->model_seller_return->getReturns(($page - 1) * $this->config->get('config_limit_admin'), $this->config->get('config_limit_admin'));
		...
		foreach ($results as $result) {
			$data['returns'][] = array(
                ...
                'href_product' => $this->url->link('product/product', 'product_id='.$result['product_id']),
				'href'       => $this->url->link('seller/return/info', 'return_id=' . $result['return_id'] . $url, 'SSL')
			);
		}
		...
		$pagination->url = $this->url->link('seller/return', 'page={page}', 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/seller/return_list.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/seller/return_list.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/seller/return_list.tpl', $data));
		}
	}
	...
	public function info() {
		...
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('seller/return/info', 'return_id=' . $return_id, 'SSL');
		...
		$return_info = $this->model_seller_return->getReturn($return_id);

		if ($return_info) {

            if ($this->request->server['REQUEST_METHOD'] == 'POST') {
                if (isset($this->request->post['return_status_id']) && ($return_info['return_status_id'] == 2 || (int)$this->request->post['return_status_id'] > $return_info['return_status_id'])) {
                    $this->request->post['notify'] = 1;
                    $this->model_seller_return->addReturnHistory($return_id, $this->request->post);
                }

                $this->response->redirect($this->url->link('seller/return/info', 'return_id=' . $return_id, 'SSL'));
            }
            ...
            $data['form_action'] = $this->url->link('seller/return/info', 'return_id=' . $return_id, 'SSL');

			$data['histories'] = array();

			$results = $this->model_seller_return->getReturnHistories($this->request->get['return_id']);
			...
			$data['continue'] = $this->url->link('seller/return', $url, 'SSL');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/seller/return_info.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/seller/return_info.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/seller/return_info.tpl', $data));
			}
		} else {
			$this->document->setTitle($this->language->get('text_return'));
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('seller/return', '', 'SSL')
			);
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('seller/return/info', 'return_id=' . $return_id . $url, 'SSL')
			);
			...
			$data['continue'] = $this->url->link('seller/return', '', 'SSL');
	...
	public function add() {die('Access denied.');
        ...
        if (!$order_info || !$product_info || !in_array($order_info['order_status_id'], array(2,3,15)) || $product_info['return_id']) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->request->post['order_id'] = $product_info['order_id'];
            $this->request->post['product_id'] = $product_info['product_id'];
            $this->request->post['order_product_id'] = $product_info['order_product_id'];
            $this->request->post['product'] = $product_info['name'];
            $this->request->post['model'] = $product_info['model'];
            $this->request->post['quantity'] = $product_info['quantity'];

			$return_id = $this->model_seller_return->addReturn($this->request->post);
			...
			$this->response->redirect($this->url->link('seller/return/success', '', 'SSL'));
		}
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('seller/return/add', '', 'SSL')
		);
		...
		$data['action'] = $this->url->link('seller/return/add', 'order_id='.$product_info['order_id'].'&product_id='.$product_info['order_product_id'], 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/seller/return_form.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/seller/return_form.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/seller/return_form.tpl', $data));
		}
	}
