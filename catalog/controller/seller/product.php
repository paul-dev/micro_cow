...
*/
	...
	public function add() {
		...
		if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('seller/product/edit', 'product_id='.$this->request->get['product_id'], 'SSL');
		...
		*/
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->post['purchase_id'])){
				...
				$company = $this->model_catalog_purchase->getCompanyInfo($this->request->post['purchase_id']);
    ...
    public function mess() {
        ...
        if (isset($this->request->post['selected']) && isset($this->request->post['action'])) {
            foreach ($this->request->post['selected'] as $product_id) {
                $_exist = $this->model_seller_product->getProduct($product_id);
                if (!empty($_exist)) {
                    if ($this->request->post['action'] == 'on_sale') {
                        $this->model_seller_product->updateProductSaleStatus($product_id, 1);
                    } elseif ($this->request->post['action'] == 'un_sale') {
                        $this->model_seller_product->updateProductSaleStatus($product_id, 0);
                    }
                }
            }
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $product_id) {
                $_exist = $this->model_seller_product->getProduct($product_id);
                if (!empty($_exist)) $this->model_seller_product->deleteProduct($product_id);
			}
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $product_id) {
                $_exist = $this->model_seller_product->getProduct($product_id);
                if (!empty($_exist)) $this->model_seller_product->copyProduct($product_id);
			}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('seller/product/add', $url, 'SSL');
		$data['copy'] = $this->url->link('seller/product/copy', $url, 'SSL');
        $data['delete'] = $this->url->link('seller/product/delete', $url, 'SSL');
        $data['mess'] = $this->url->link('seller/product/mess', $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'status'     => ($result['status']) ? ($result['on_sale'] ? '销售中' : '已下架') : '待审核',
                'href'       => $this->url->link('product/product', 'preview=1&product_id=' . $result['product_id']),
				'edit'       => $this->url->link('seller/product/edit', 'product_id=' . $result['product_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_status'] = $this->url->link('seller/product', 'sort=p.status' . $url, 'SSL');
	...
	protected function getForm() {
		...
		$data['recurrings'] = $this->model_seller_recurring->getRecurrings();
		...
		foreach ($categories as $category_id) {
			...
			if ($category_info) {
                ...
                $data['select_categories'][] = $category_id;
            }
		}
        ...
        $system_categories = $this->model_catalog_category->getSystemCategoriesTree(array(
            'store_id' => 0,
            'sort' => 'sort_order'
        ));
        foreach ($system_categories as $category) {
            if (!$category['status']) continue;
            $data['system_categories'][] = $category;
            //echo $category['sort_order'] . ' - ' . $category['category_id'] . ' - ' . $category['name'] . '<br/>';
        }
		...
		foreach ($filters as $filter_id) {
			...
			if ($filter_info) {
				$data['product_filters'][$filter_info['filter_id']] = array(
        ...
        foreach ($results as $result) {
            ...
            $data['attribute_groups'][$result['attribute_group_id']] = array(
		...
		foreach ($product_attributes as $product_attribute) {
			...
			if ($attribute_info) {
                ...
                if (isset($data['attribute_groups'][$attribute_info['attribute_group_id']])) {
                    $data['attribute_groups'][$attribute_info['attribute_group_id']]['selected'] = true;
                }
			}
		}
        ...
        if (isset($this->request->post['product_auctions'])) {
            $product_auctions = $this->request->post['product_auctions'];
        } elseif (isset($this->request->get['product_id'])) {
            $product_auctions = $this->model_seller_product->getProductAuctions($this->request->get['product_id']);
        } else {
            $product_auctions = array();
        }

        $data['product_auctions'] = array();

        foreach ($product_auctions as $product_auction) {
            $data['product_auctions'][] = array(
                'auction_id'        => $product_auction['product_auction_id'],
                'customer_group_id' => $product_auction['customer_group_id'],
                'priority'          => $product_auction['priority'],
                'base_price'        => $product_auction['base_price'],
                'price'             => $product_auction['price'],
                'date_start'        => ($product_auction['date_start'] != '0000-00-00 00:00:00') ? $product_auction['date_start'] : '',
                'date_end'          => ($product_auction['date_end'] != '0000-00-00 00:00:00') ? $product_auction['date_end'] :  ''
            );
        }
		...
		if (isset($this->request->post['product_reward'])) {
			...
			$data['product_reward'] = $this->model_seller_product->getProductRewards($this->request->get['product_id']);
		} else {
