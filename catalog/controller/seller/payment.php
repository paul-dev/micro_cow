<?php
class ControllerSellerPayment extends Controller {
	private $error = array();
	...
	public function index() {
        ...
        $this->load->language('seller/payment');
	...
	public function getList() {
		...
		if ($files) {
			foreach ($files as $file) {
				...
				$data['extensions'][] = array(
					...
					'sort_order' => $_sort,
					'install'   => $this->url->link('seller/payment/install', 'extension=' . $extension, 'SSL'),
					'uninstall' => $this->url->link('seller/payment/uninstall', 'extension=' . $extension, 'SSL'),
					'installed' => in_array($extension, $extensions),
					'edit'      => $this->url->link('payment/' . $extension . '/edit', '', 'SSL')
				);
			}
		}
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/seller/payment.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/seller/payment.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/seller/payment.tpl', $data));
        }
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'seller/payment')) {
