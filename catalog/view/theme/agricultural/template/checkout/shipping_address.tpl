<form class="form-horizontal">
  ...
  <div id="shipping-existing">
    <select name="address_id" class="form-control">
      ...
      <option value="<?php echo $address['address_id']; ?>" selected="selected"><?php echo $address['fullname']; ?>, <?php if(isset($address['custom_field'][1])) echo $address['custom_field'][1].', '; ?><?php echo $address['address']; ?>, <?php echo $address['city']; ?><?php if ($address['postcode']) echo '('.$address['postcode'].')'; ?>, <?php echo $address['zone']; ?>, <?php echo $address['country']; ?></option>
      <?php } else { ?>
      <option value="<?php echo $address['address_id']; ?>"><?php echo $address['fullname']; ?>, <?php echo $address['address']; ?>, <?php echo $address['city']; ?><?php if ($address['postcode']) echo '('.$address['postcode'].')'; ?>, <?php echo $address['zone']; ?>, <?php echo $address['country']; ?></option>
      <?php } ?>
      <?php } ?>
    </select>
  </div>
  ...
  <div id="shipping-new" style="display: <?php echo ($addresses ? 'none' : 'block'); ?>;">
    ...
    <div class="form-group required">
      ...
      <div class="form-group required">
          <label class="col-sm-2 control-label" for="input-shipping-area">区县</label>
          <div class="col-sm-10">
              <select name="area_id" id="input-shipping-area" class="form-control">
              </select>
          </div>
      </div>
    ...
    <?php } ?>
      ...
      <div class="buttons clearfix">
          <div class="pull-right">
              <input type="button" value="保存" id="button-shipping-address" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary" />
          </div>
      </div>
            ...
            success: function(json) {
                ...
                if (json['area'] && json['area'] != '') {
                    for (var i = 0; i < json['area'].length; i++) {
                        ...
                        if (json['area'][i]['id'] == '<?php echo $area_id; ?>') {
                            html += ' selected="selected"';
                        }
                    ...
                    html += '<option value="0" selected="selected"><?php echo $text_none; ?></option>';
                    var _city = $('#collapse-shipping-address select[name=\'zone_id\']').find('option:selected').text();
                    $('#collapse-shipping-address input[name=\'city\']').val(_city);
                }
                if (json['name']) $('#collapse-shipping-address input[name=\'city\']').val(json['name']);
                $('#collapse-shipping-address select[name=\'area_id\']').html(html);
            },
            error: function(xhr, ajaxOptions, thrownError) {
                alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
            }
