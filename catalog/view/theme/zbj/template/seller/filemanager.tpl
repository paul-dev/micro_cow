    ...
    <div class="modal-body">
      <div class="row">
        <div class="col-sm-5"><a href="<?php echo $parent; ?>" data-toggle="tooltip" title="<?php echo $button_parent; ?>" id="button-parent" class="btn btn-default"><i class="fa fa-level-up"></i></a> <a href="<?php echo $refresh; ?>" data-toggle="tooltip" title="<?php echo $button_refresh; ?>" id="button-refresh" class="btn btn-default"><i class="fa fa-refresh"></i></a>
          ...
          <button type="button" data-toggle="tooltip" title="<?php echo $button_delete; ?>" id="button-delete" class="btn btn-danger"><i class="fa fa-trash-o"></i></button>
            <?php if (!$target) { ?>
            <button type="button" data-toggle="tooltip" title="批量插入" id="button-insert" class="btn btn-primary"><i class="fa fa-plus"></i></button>
            <?php } ?>
        </div>
        <div class="col-sm-7">
      ...
      <div class="row">
        ...
        <div class="col-sm-3 text-center">
          ...
          <a href="<?php echo $image['href']; ?>" data-path="<?php echo $image['sub_path']; ?>" class="thumbnail"><img src="<?php echo $image['thumb']; ?>" alt="<?php echo $image['name']; ?>" title="<?php echo $image['name']; ?>" /></a>
          <label>
            <input type="checkbox" name="path[]" value="<?php echo $image['path']; ?>" />
            <?php echo $image['name']; ?></label>
          <?php } ?>
...
$('a.thumbnail').on('click', function(e) {
	...
	//$('#<?php echo $target; ?>').attr('value', $(this).parent().find('input').attr('value'));
        $('#<?php echo $target; ?>').attr('value', _path);
	<?php } else { ?>
...
$('#modal-image #button-delete').on('click', function(e) {
    ...
    $('#modal-image #button-insert').on('click', function(e) {
        if ($('input[name^=\'path\']:checked').length == 0) {
            ...
            $('input[name^=\'path\']:checked').each(function(){
                ...
                if (sel.rangeCount) {
                    var img = document.createElement('img');
                    img.src = $(this).parent().parent().find('a.thumbnail').attr('href');
