...
function main(source) {
  ...
  for (var i = 0; i < source.length; i++) {
    str += "<img src=" + source[i] + " width=\"980px\" height=\"428px\" />"
  }
  gallery_yyl.innerHTML = str;
  gallery_yyl.style.width = 980 * source.length + "px";//设置图片集的宽度
  //btn的生成
  ...
  btn_left.onclick = function () {
    ...
    for (var i = 0; i < source.length; i++) {
      if (galL == -980 * i) {
        if (galL == 0) {
          onMove(gallery_yyl, {left: -980 * source.length + 980}, 5);
        } else {
          onMove(gallery_yyl, {left: galL + 980}, 5);
        }
      }
    }
  }
...
function timeMove(obj, arr) {
  ...
  timer = setInterval(function () {
    ...
    if (galL == -980 * arr.length + 980) {
      onMove(obj, {left: 0}, 5);
    } else {
      onMove(obj, {left: galL - 980}, 5);
    }
  }, 3000);
  return timer;
}
...
function  changeimg(e){
  var a=$(e).attr("src");
  var b=$(e).parents(".album-body");
  var c= b.find(".mian-album-img");
  $(c).attr("src",a);
}
...
!function (a) {
  a.fn.slide = function (b) {
    return a.fn.slide.defaults = {
      ...
      titOnClassName: "on",
      autoPage: !1,
      prevCell: ".banner_zy .prev",
      nextCell: ".banner_zy .next",
